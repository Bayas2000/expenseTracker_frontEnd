...
const IncomeAdd = ({ setOpenAddModal, setActiveTab }) => {
  const IncomeModal = useSelector((state) => state.banner?.Income_Modal_List);
  ...
  React.useEffect(() => {
    if (IncomeModal) {
      setDetails({
        ...
        notes: IncomeModal.description || "",
      });
      setDate(IncomeModal.transactionDate?.substring(0, 10) || "");
      setCategory({
        value: IncomeModal.categoryId,
        label: IncomeModal.categoryName,
        icon: IncomeModal.categoryIcon,
      });
    }
  }, [IncomeModal]);
  ...
  return (
    <div className="px-4">
      ...
      <div className=" flex items-center mb-2 ">
        <button
          onClick={
            IncomeModal && IncomeModal?._id ? handleUpdate : handleSubmit
          }
          className={`${
            IncomeModal && IncomeModal?._id ? "bg-blue-500" : "bg-green-500"
          } text-white pl-4 py-1 rounded-l-lg mt-4 w-full border border-gray-200 shadow-lg `}
        >
          {IncomeModal && IncomeModal?._id ? "Update Income" : "Add Income"}
        </button>
        {IncomeModal && IncomeModal?._id && (
          <button
